...
To prepare that publish step without committing or pushing, use `node Extension/scripts/prepare-recycle-config-candidate-publish.js --input <candidate.json> --config-repo <path/to/oss-assistant-config>`. The helper validates/reviews the external candidate, stops on `REVIEW_REQUIRED` unless `--accept-review-required` is passed after human acceptance, copies only to `<oss-assistant-config>/config/recycle-device-catalog.json`, and runs the public validator/diff checks. It does not commit, push, copy images, copy `assets-manifest.json`, copy configurator UI, or touch runtime files.
