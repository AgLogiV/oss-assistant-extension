...
3. **Optional validated remote overlay**
   ...
   - Refresh policy:
     ...
     - `Enable material` remains available as a manual/debug diagnostic action. Automatic remote material/SAP is now implemented separately (`c4852bf Add automatic remote recycle material enablement`): it requires both `remoteAdditionsAuto` and `remoteMaterialAuto`, applies only to already auto-accepted remote devices, reruns content-side material eligibility, uses a separate same-tab `sessionStorage` projection `wifi_oss_recycle_remote_auto_session_state_v1` for `swap-material` navigation, and does not trust remote `generatedMaterialFilters`.
   ...
   - Automatic remote cards/material are capability-gated and local-first. Production public JSON still omits `runtimeContract` and remains fallback-only; debug/staging sources can exercise auto cards/material only when the remote contract explicitly advertises `remoteAdditionsAuto` and `remoteMaterialAuto`.
