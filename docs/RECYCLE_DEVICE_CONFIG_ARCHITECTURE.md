...
2. **GitHub or simple static hosting as the first remote MVP**
   ...
   - Practical smoke-tested package command: `node Extension/scripts/export-recycle-static-config-package.js --out path/to/output-dir --include-images`. Current output is `2` config files plus `59` images (`41` device images and `18` help images), `61` files total. The generated catalog should pass `validate-recycle-config-fixture.js --input`, `review-recycle-config-candidate.js --input` should return PASS for the current generated catalog, and every manifest image should exist in the package with safe extension-relative paths.
