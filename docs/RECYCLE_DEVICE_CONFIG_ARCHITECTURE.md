...
3. **Optional validated remote overlay**
   ...
   - Automatic remote cards/material are capability-gated and local-first. Production public JSON still omits `runtimeContract` and remains fallback-only; debug/staging sources can exercise auto cards/material only when the remote contract explicitly advertises `remoteAdditionsAuto` and `remoteMaterialAuto`. Remote config can introduce a new SAP/material ID only through `remoteMaterialModelsAuto` plus top-level `remoteMaterialModels`; each model is strictly validated and bound to an eligible remote device by `materialId`, `deviceId`, `categoryId`, and safe `name`.
   ...
   - Controlled auto-card/auto-material staging smoke passed using public staging fixture `https://oss-assistant.github.io/oss-assistant-config/staging/recycle-device-catalog.test.json` after public commits `3965ee8 Enable staging remote additions auto capability` and `3e1dd26 Enable staging remote material auto capability`: `Use debug source` plus `Refresh remote` showed `source debug`, `contract v1 ok`, `auto remote applied 1`, `auto material 1`, `fresh`, and `LKG yes`; neither `Apply eligible` nor `Enable material` was pressed; the router category showed `STAGING TEST REMOTE ROUTER / 123451`. `staging/recycle-device-catalog.test.json` is the stable smoke/regression fixture. Future visual demos should use a separate demo fixture, production remains default/fallback, and remote `generatedMaterialFilters` is not runtime authority.
   - Controlled remote material model demo smoke passed using `https://oss-assistant.github.io/oss-assistant-config/staging/recycle-device-catalog.demo.json` after private commits `86d7925 Add remote material model extension support` and `9e27760 Fix remote capability detection`, plus public commit `e9684a9 Add remote material models demo fixture`: after extension reload, `Clear`, `Use debug source`, and `Refresh remote`, the tray showed `source debug`, `contract v1 ok`, `auto remote applied 1`, `auto material 1`, `fresh`, and `LKG yes`; neither `Apply eligible` nor `Enable material` was pressed; `routers` showed `DEMO Remote Router 111111 / 111111`. The fix ensures `supportedCapabilities` is not logically truncated; sample limits are only for warnings/errors/debug samples.
   - Staged rollout:
     1. docs/spec;
