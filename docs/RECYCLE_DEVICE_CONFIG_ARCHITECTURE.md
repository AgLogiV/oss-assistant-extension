...
3. **Optional validated remote overlay**
   ...
   - Refresh policy:
     ...
     - `Source` controls are manual/debug-only (`834b964 Add remote config debug source override`). Production remains the default source. `Use debug source` accepts only allowlisted GitHub Pages JSON under `https://oss-assistant.github.io/oss-assistant-config/`; `Use production` clears the override. Setting or clearing the source clears LKG/meta/status while preserving Auto-refresh ON/OFF, and status/preview/apply paths ignore cached LKG when `meta.sourceUrl` does not match the active source.
     - `Apply visual` is always manual and is never triggered by auto-refresh or preview.
     - `Apply eligible` is also manual/debug-only. It uses the apply-safe `recycleConfig.getResolvedCatalogApplyPlan` projection, then content-side revalidation, to place eligible unknown remote devices into an in-memory added-device overlay as temporary UI cards only.
     - `Enable material` is manual/debug-only and applies only to already `Apply eligible` remote-added devices that pass strict known-material gates. It also uses `recycleConfig.getResolvedCatalogApplyPlan`, intersects material candidates with the current in-memory added-device overlay, reruns content-side material eligibility, and persists a minimal same-tab debug projection in `sessionStorage` key `wifi_oss_recycle_remote_debug_session_state_v1` so OSS navigation/full reload to `swap-material` can rehydrate before the material observer; it does not store the full remote catalog and does not trust remote `generatedMaterialFilters`.
     ...
     - Later, first recycle panel use may trigger a stale-cache refresh only after production timing/status UX is reviewed.
   ...
   - Current remote JSON capability map:
     ...
     - Local-only authority: validation profile implementations, category semantics, CAM/modems special flow, SAP/material model data, legacy material rewrite behavior, DOM selectors, navigation, clipboard, labels/barcodes, dashboard/API, and `rewriteMap`.
   ...
   - Future commit boundaries for this direction:
     ...
     5. keep source override manual/debug-only while later dashboard/configurator emit support is designed. Staging source override is now available for cheaper tests, but production remains the default source and no production auto-apply is implied.
   ...
   - Controlled debug source override and staging catalog smoke passed using public staging fixture `f9dcbbf Add staging recycle catalog test fixture` at `https://oss-assistant.github.io/oss-assistant-config/staging/recycle-device-catalog.test.json`: `Use debug source` switched to `source debug`, `Refresh remote` loaded the staging source with `contract v1 ok` and `LKG yes`, `Preview plan` showed `add 1`, `material 1`, `blocked 0`, `Apply eligible` added `staging_test_remote_router`, `Enable material` enabled material `123451`, the router category showed `STAGING TEST REMOTE ROUTER / 123451`, `Clear` removed the staging card, and `Use production` returned to `source production` with production `contract legacy ok` and `LKG yes`. Staging remains manual/debug-only, production remains default, and remote `generatedMaterialFilters` is not runtime authority.
   - Staged rollout:
     1. docs/spec;
