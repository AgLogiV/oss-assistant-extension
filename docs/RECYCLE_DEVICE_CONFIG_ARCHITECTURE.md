...
3. **Optional validated remote overlay**
   ...
   - Refresh policy:
     ...
     - `Apply eligible` is also manual/debug-only. It can place eligible unknown remote devices into an in-memory added-device overlay as temporary UI cards only.
     - `Enable material` is manual/debug-only and applies only to already `Apply eligible` remote-added devices that pass strict known-material gates. It persists a minimal same-tab debug projection in `sessionStorage` key `wifi_oss_recycle_remote_debug_session_state_v1` so OSS navigation/full reload to `swap-material` can rehydrate before the material observer; it does not store the full remote catalog and does not trust remote `generatedMaterialFilters`.
   ...
   - Chosen next runtime direction after debug material enablement: build a central resolved remote catalog plan before adding staging/source selection or production-like apply. The plan should project cached remote LKG plus local context into compact, side-effect-free sections such as visual updates, eligible additions, material-eligible additions, blocked entries, warnings, and `sourceRevision`. It must not return raw/full remote devices and must not apply anything by itself.
   - Future commit boundaries for this direction:
     1. preview-only resolved plan in background/content/debug tray;
     2. route `Apply eligible` and `Enable material` through the same resolved plan while preserving current manual/debug behavior;
     3. update docs/checklist after smoke;
     4. add staging/source selection later. Staging/source selection is useful for cheaper tests, but it is secondary to making the JSON runtime contract explicit and reusable.
