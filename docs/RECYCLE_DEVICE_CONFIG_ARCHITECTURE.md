...
3. **Optional validated remote overlay**
   ...
   - Refresh policy:
     ...
     - External simple source adapter support is checkpointed on branch `codex/external-recycle-catalog-runtime` (`464b564 Add external recycle catalog runtime adapter`). The tested raw GitHub source is not the full OSS remote schema; it is detected as `external_simple_v1`, normalized in `background.js`, and internally synthesizes `schemaVersion`, `revision`, `runtimeContract`, generated-material metadata, and `remoteMaterialModels`. The existing full OSS schema path remains unchanged. The intended next direction is a separate guarded patch that can make an approved external source the default remote source for newly added devices while preserving source-scoped LKG/local fallback and manual force refresh. See `docs/EXTERNAL_RECYCLE_CATALOG_RUNTIME.md`.
