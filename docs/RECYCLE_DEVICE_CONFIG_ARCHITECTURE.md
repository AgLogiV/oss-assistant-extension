...
3. **Optional validated remote overlay**
   ...
   - Runtime contract compatibility reporting is implemented (`6040a8e Add remote runtime contract reporting`). `background.js` has internal `RECYCLE_REMOTE_RUNTIME_CONTRACT` and `recycleRemoteEvaluateCatalogContract(...)`; status, `Preview plan`, and the resolved apply-safe plan include compact `contractCompatibility`. The current public JSON has no `runtimeContract` and is treated as `legacy_v1` compatible, shown as `contract legacy ok`. A future optional `runtimeContract` can be evaluated, but this stage is reporting-only: it does not block apply flows, change public JSON, update validators, or change `Apply visual`, `Apply eligible`, `Enable material`, `Clear`, `sessionStorage`, or SAP/material behavior. Unsupported `schemaVersion` and missing `revision` remain hard validation failures.
   - Future commit boundaries for this direction:
     1. add optional `runtimeContract` validator/script support before any dashboard/configurator emits it;
     2. add apply-plan contract guards only after validator support and browser smoke;
     3. add staging/source selection later. Staging/source selection is useful for cheaper tests, but it is secondary to making the JSON runtime contract explicit and reusable.
