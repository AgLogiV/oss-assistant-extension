...
- The browser configurator still does not write to GitHub. The runtime extension now has manual/debug remote recycle config stages: `background.js` fetches/validates/caches the GitHub Pages catalog in `chrome.storage.local`, `content.js` exposes a CSP-safe debug bridge, and Stage 4 can manually apply an in-memory visual-only overlay. The recycle panel also has a collapsed `Remote config debug` tray for `Source`, `Auto-refresh`, `Status`, `Refresh remote`, `Preview diff`, `Preview plan`, `Apply visual`, `Apply eligible`, `Enable material`, and `Clear` during manual testing. `Source` is a manual/debug override limited to allowlisted GitHub Pages JSON under `https://oss-assistant.github.io/oss-assistant-config/`; production remains the default source and source changes clear LKG/meta/status to avoid cross-source cache reuse. `Preview diff` compares cached remote LKG with the packaged local catalog and shows compact visual/risky/unknown/missing plus unknown-device eligibility summaries without applying anything or exposing full raw catalog data. `Preview plan` uses compact/capped `recycleConfig.getResolvedCatalogPlan`, while `Apply eligible` and `Enable material` use apply-safe `recycleConfig.getResolvedCatalogApplyPlan` projected entries plus content-side revalidation. Contract compatibility is reported compactly through status/plan/apply-plan; the current public JSON without `runtimeContract` is `legacy_v1` compatible, and incompatible contracts block only manual `Apply eligible` / `Enable material`. Optional `runtimeContract` is now accepted by private validators/review scripts and public config tools, and the public static configurator preserves it read-only on load/export if present. Production public JSON still omits `runtimeContract`, and dashboard/configurator emit support is not enabled yet. `Apply eligible` can manually add only eligible unknown remote devices as in-memory temporary UI cards. `Enable material` is debug/manual-only for already-added eligible remote devices; it uses a minimal same-tab `sessionStorage` debug projection so OSS navigation to `swap-material` can rehydrate, but it does not mutate `RECYCLE_DEVICE_CATALOG`, does not rebuild `SWAP_MATERIAL_RECYCLE_FILTERS`, and does not trust remote `generatedMaterialFilters`. Auto-refresh is default OFF, TTL-gated at 6 hours when ON, and updates cache/status only; `Apply visual`, `Apply eligible`, and `Enable material` remain manual. There is still no startup auto-apply, periodic/scheduled refresh, visible production rollout UI, or broad remote catalog replacement.
