...
- `node Extension/scripts/export-recycle-static-config-package.js --out path/to/output-dir` is the dev-only static package exporter. It writes only to the explicit output directory, validates the generated catalog, refuses runtime/source output paths, and does not copy images/configurator UI or create GitHub Pages files yet.
