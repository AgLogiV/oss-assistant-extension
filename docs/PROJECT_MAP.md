...
- The browser configurator still does not write to GitHub. The runtime extension now has manual/debug remote recycle config stages: `background.js` fetches/validates/caches the GitHub Pages catalog in `chrome.storage.local`, `content.js` exposes a CSP-safe debug bridge, and Stage 4 can manually apply an in-memory visual-only overlay. The recycle panel also has a collapsed `Remote config debug` tray for `Auto-refresh`, `Status`, `Refresh remote`, `Preview diff`, `Preview plan`, `Apply visual`, `Apply eligible`, `Enable material`, and `Clear` during manual testing. `Preview diff` compares cached remote LKG with the packaged local catalog and shows compact visual/risky/unknown/missing plus unknown-device eligibility summaries without applying anything or exposing full raw catalog data. `Preview plan` uses compact/capped `recycleConfig.getResolvedCatalogPlan`, while `Apply eligible` and `Enable material` use apply-safe `recycleConfig.getResolvedCatalogApplyPlan` projected entries plus content-side revalidation. Contract compatibility is now reported compactly through status/plan/apply-plan; the current public JSON without `runtimeContract` is `legacy_v1` compatible, and future optional `runtimeContract` support is reporting-only until validators are updated. `Apply eligible` can manually add only eligible unknown remote devices as in-memory temporary UI cards. `Enable material` is debug/manual-only for already-added eligible remote devices; it uses a minimal same-tab `sessionStorage` debug projection so OSS navigation to `swap-material` can rehydrate, but it does not mutate `RECYCLE_DEVICE_CATALOG`, does not rebuild `SWAP_MATERIAL_RECYCLE_FILTERS`, and does not trust remote `generatedMaterialFilters`. Auto-refresh is default OFF, TTL-gated at 6 hours when ON, and updates cache/status only; `Apply visual`, `Apply eligible`, and `Enable material` remain manual. There is still no startup auto-apply, periodic/scheduled refresh, visible production rollout UI, or broad remote catalog replacement.
...
- `Extension/scripts/validate-recycle-config-fixture.js --input path/to/candidate.json` - dev-only candidate JSON validator for future local configurator exports. It validates schema/shape/data without requiring exact parity with current `Extension/content.js`; the extension runtime still does not load JSON config.
- `Extension/scripts/review-recycle-config-candidate.js --input path/to/candidate.json` - dev-only no-write candidate review helper. It compares exported candidate JSON to the current runtime-shaped export by stable `deviceId`, reports added/edited/missing/reordered devices, material filter changes, unknown fields, and manual-review-only sections, but does not merge, write `content.js`, regenerate the fixture, or add runtime JSON loading.
