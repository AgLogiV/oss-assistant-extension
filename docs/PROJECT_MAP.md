...
- The browser configurator still does not write to GitHub. The runtime extension now has manual/debug remote recycle config stages: `background.js` fetches/validates/caches the GitHub Pages catalog in `chrome.storage.local`, `content.js` exposes a CSP-safe debug bridge, and Stage 4 can manually apply an in-memory visual-only overlay. The recycle panel also has a collapsed `Remote config debug` tray for `Auto-refresh`, `Status`, `Refresh remote`, `Preview diff`, `Apply visual`, `Apply eligible`, `Enable material`, and `Clear` during manual testing. `Preview diff` compares cached remote LKG with the packaged local catalog and shows compact visual/risky/unknown/missing plus unknown-device eligibility summaries without applying anything or exposing full raw catalog data. `Apply eligible` can manually add only eligible unknown remote devices as in-memory temporary UI cards. `Enable material` is debug/manual-only for already-added eligible remote devices; it uses a minimal same-tab `sessionStorage` debug projection so OSS navigation to `swap-material` can rehydrate, but it does not mutate `RECYCLE_DEVICE_CATALOG`, does not rebuild `SWAP_MATERIAL_RECYCLE_FILTERS`, and does not trust remote `generatedMaterialFilters`. The chosen next runtime direction is a central preview-only resolved remote catalog plan so the remote JSON contract becomes explicit before staging/source selection or production-like apply. Auto-refresh is default OFF, TTL-gated at 6 hours when ON, and updates cache/status only; `Apply visual`, `Apply eligible`, and `Enable material` remain manual. There is still no startup auto-apply, periodic/scheduled refresh, visible production rollout UI, or broad remote catalog replacement.
