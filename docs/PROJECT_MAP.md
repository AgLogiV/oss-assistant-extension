...
- The runtime extension still does not load remote config. Remote config overlay remains a future separate task, and the browser configurator still does not write to GitHub. The planned overlay is local-first: future `background.js` fetches/validates/caches with `chrome.storage.local`, while `content.js` consumes only sanitized data by message passing after explicit rollout.
...
- `Extension/background.js` - MV3 service worker. Handles toolbar-click injection and proxies dashboard fetches/image downloads for `content.js`. A future optional remote recycle config overlay should put GitHub Pages fetch, timeout, ETag/status, validation, and `chrome.storage.local` last-known-good cache here, not directly in page UI code.
