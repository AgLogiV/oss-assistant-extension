...
- The browser configurator still does not write to GitHub. The runtime extension now has manual/debug remote recycle config stages: `background.js` fetches/validates/caches the GitHub Pages catalog in `chrome.storage.local`, `content.js` exposes a CSP-safe debug bridge, and Stage 4 can manually apply an in-memory visual-only overlay. The recycle panel also has a collapsed `Remote config debug` tray for `Auto-refresh`, `Status`, `Refresh remote`, `Preview diff`, `Apply visual`, `Apply eligible`, and `Clear` during manual testing. `Preview diff` compares cached remote LKG with the packaged local catalog and shows compact visual/risky/unknown/missing plus unknown-device eligibility summaries without applying anything or exposing full raw catalog data. `Apply eligible` can manually add only eligible unknown remote devices as in-memory temporary UI cards; it does not enable SAP/material behavior, does not mutate `RECYCLE_DEVICE_CATALOG`, and does not rebuild `SWAP_MATERIAL_RECYCLE_FILTERS`. Next direction is an explicit effective catalog helper layer with local packaged catalog as immutable base/fallback; the first patch should not add SAP/material behavior. Auto-refresh is default OFF, TTL-gated at 6 hours when ON, and updates cache/status only; `Apply visual` and `Apply eligible` remain manual. There is still no startup auto-apply, periodic/scheduled refresh, visible production rollout UI, or broad remote catalog replacement.
