...
- The browser configurator still does not write to GitHub. The runtime extension now has manual/debug remote recycle config stages: `background.js` fetches/validates/caches the GitHub Pages catalog in `chrome.storage.local`, `content.js` exposes a CSP-safe debug bridge, and Stage 4 can manually apply an in-memory visual-only overlay. The recycle panel also has a collapsed `Remote config debug` tray for `Status`, `Refresh remote`, `Apply visual`, and `Clear` during manual testing. There is still no startup auto-apply, periodic refresh, visible production rollout UI, or broad remote catalog replacement.
