...
- `Extension/tools/recycle-configurator/` - dev-only local recycle configurator. Start with `start-configurator.cmd` or `node Extension/tools/recycle-configurator/server.js`; it loads the fixture, supports search/category filtering, compact device selection, side-editor browser-memory edits, asset selectors/previews, candidate validation, candidate JSON export, and revert. It has no server-side save/write endpoint and is not used by extension runtime.
