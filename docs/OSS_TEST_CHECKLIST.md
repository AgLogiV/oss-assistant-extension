...
- The normal manual remote-config test surface is the collapsed `Remote config debug` tray inside the extension-owned recycle panel, under the category/device grid. Expand it and check `Status`, `Refresh remote`, `Apply visual`, and `Clear`; the tray replaces the need to paste the page-console helper for ordinary manual tests. It does not auto-refresh on panel open, does not auto-apply on page load, and returns to `not checked` after page refresh.
