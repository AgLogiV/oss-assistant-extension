...
- Next remote-added-device architecture direction is an effective catalog layer: keep the packaged catalog as base/fallback and keep remote-added devices in an in-memory overlay. Until a separate material enablement stage is reviewed, `SWAP_MATERIAL_RECYCLE_FILTERS`, `getSwapMaterialRecycleFilter`, `getRecycleDeviceImagePathByMaterialId`, material snapshots, controlled fill, and SAP/material quick buttons must remain local-only. `Clear` removes remote-added cards and resets preview state.
