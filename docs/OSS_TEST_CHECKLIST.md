...
- The normal manual remote-config test surface is the collapsed `Remote config debug` tray inside the extension-owned recycle panel, under the category/device grid. Expand it and check `Auto-refresh`, `Status`, `Refresh remote`, `Apply visual`, and `Clear`; the tray replaces the need to paste the page-console helper for ordinary manual tests. Auto-refresh is default OFF, uses a hardcoded 6-hour TTL when ON, and only lets `Status` perform TTL-gated maybe-refresh. `Refresh remote` remains manual force refresh, `Apply visual` remains manual, and `Clear` removes cache/LKG/meta/status plus in-memory overlay while preserving Auto-refresh ON/OFF. There is no auto-refresh on panel open, startup/page-load fetch, periodic/scheduled refresh, `chrome.alarms`, or auto-apply on page load.
