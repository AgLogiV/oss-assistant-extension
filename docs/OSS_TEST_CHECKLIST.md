...
- External simple adapter smoke (`464b564 Add external recycle catalog runtime adapter` on branch `codex/external-recycle-catalog-runtime`): use `Source` -> `Use debug source` with `https://raw.githubusercontent.com/AgLogiV/oss-assistant-extension/main/config/recycle-device-catalog.fixture.json`, then `Refresh remote`. Expected successful smoke is `source external`, `contract v1 ok`, `auto remote applied 2`, `auto material 2`, `fresh`, `LKG yes`, visible `BOJIDAT NETBOX / 888888` and `BOJKATA RUTERA BRAT / 9191919191`, external images rendered, SAP/material `888888` filled for `BOJIDAT NETBOX`, selected remote-added device persists after page refresh, and local plus remote selected devices both persist after refresh. The file is simple `external_simple_v1`, not the full OSS schema; `background.js` normalizes it internally and generates `remoteMaterialModels`. See `docs/EXTERNAL_RECYCLE_CATALOG_RUNTIME.md` before making it a default source.
