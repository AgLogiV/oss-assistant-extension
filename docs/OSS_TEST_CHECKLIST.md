...
- For a future static config package preview, run `node Extension/scripts/export-recycle-static-config-package.js --out path/to/output-dir` with optional `--dry-run` or `--force`. It is dev-only, writes only to the explicit output directory, currently outputs `config/recycle-device-catalog.json` and `config/assets-manifest.json`, validates the generated catalog, refuses runtime/source folders, and does not copy images/configurator UI, create GitHub Pages files, or add runtime JSON loading.
