...
- For a future static config package preview, run `node Extension/scripts/export-recycle-static-config-package.js --out path/to/output-dir` with optional `--dry-run`, `--force`, or `--include-images`. It is dev-only, writes only to the explicit output directory, outputs `config/recycle-device-catalog.json` and `config/assets-manifest.json` by default, validates the generated catalog, and refuses runtime/source folders. With `--include-images`, it also copies only `assets-manifest.json` referenced package images into `images/devices/16x9/` and `images/recycle-help/`; it still does not copy configurator UI, create GitHub Pages files, add runtime JSON loading, or change runtime behavior.
- Practical static package smoke check: run `node Extension/scripts/export-recycle-static-config-package.js --out path/to/output-dir --include-images` outside the repo. Expected current structure is `config/recycle-device-catalog.json`, `config/assets-manifest.json`, `images/devices/16x9/`, and `images/recycle-help/` with `2` config files plus `59` images (`41` device images and `18` help images), `61` files total. Then run `node Extension/scripts/validate-recycle-config-fixture.js --input path/to/output-dir/config/recycle-device-catalog.json` and `node Extension/scripts/review-recycle-config-candidate.js --input path/to/output-dir/config/recycle-device-catalog.json`; the current generated catalog should return PASS, manifest paths should be safe, and every manifest image should have a copied file.
