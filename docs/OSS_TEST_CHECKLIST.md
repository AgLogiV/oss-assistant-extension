...
- Future remote overlay checks are not active yet. When implemented, first verify fetch/validate/cache without apply: `background.js` should own GitHub Pages fetch, timeout, ETag/status, validation, and `chrome.storage.local` keys `wifi_oss_recycle_remote_config_lkg_v1`, `wifi_oss_recycle_remote_config_meta_v1`, `wifi_oss_recycle_remote_config_status_v1`, and `wifi_oss_recycle_remote_config_enabled_v1`; `content.js` should stay local-first and consume only sanitized data through message passing.
- Future overlay apply checks must confirm remote omissions do not delete local devices, invalid/offline remote config falls back to the packaged catalog, first apply phase changes only existing-device visual/help metadata (`displayName`, `imagePath`, `helpImagePath`, `warningText`), and remote JSON cannot define JS, regex, selectors, OSS navigation, clipboard parsers, labels/barcodes, CAM flow, auto-continue, `rewriteMap`, keyboard normalization, dashboard polling, or new validation logic.
...
- Do not enable remote recycle overlay apply until the no-apply fetch/cache stage passes and the intentional manifest changes for `storage` plus GitHub Pages host permission have been reviewed.
