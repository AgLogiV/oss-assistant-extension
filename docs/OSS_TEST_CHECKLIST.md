...
- The normal manual remote-config test surface is the collapsed `Remote config debug` tray inside the extension-owned recycle panel, under the category/device grid. Expand it and check `Auto-refresh`, `Status`, `Refresh remote`, `Preview diff`, `Apply visual`, and `Clear`; the tray replaces the need to paste the page-console helper for ordinary manual tests. Auto-refresh is default OFF, uses a hardcoded 6-hour TTL when ON, and only lets `Status` perform TTL-gated maybe-refresh. `Refresh remote` remains manual force refresh. `Preview diff` is debug/preview-only: it compares cached remote LKG against the packaged local catalog, shows compact counts/samples for visual changes, risky changes, unknown remote devices, and missing local devices, and applies nothing. `Apply visual` remains manual, and `Clear` removes cache/LKG/meta/status plus in-memory overlay while preserving Auto-refresh ON/OFF. There is no auto-refresh on panel open, startup/page-load fetch, periodic/scheduled refresh, `chrome.alarms`, or auto-apply on page load.
...
- Unknown remote devices remain report-only. Before any future runtime support, verify a preview-only eligibility step that keeps the packaged catalog as fallback, does not mutate `RECYCLE_DEVICE_CATALOG`, does not rebuild `SWAP_MATERIAL_RECYCLE_FILTERS`, blocks `cam_modules`/`modems` additions first, requires locally implemented `validationProfileId`, checks safe/known material behavior, handles missing image/help assets with fallback, and never creates a runtime card for `enabled: false`.
