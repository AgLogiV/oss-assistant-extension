...
- The normal manual remote-config test surface is the collapsed `Remote config debug` tray inside the extension-owned recycle panel, under the category/device grid. Expand it and check `Auto-refresh`, `Status`, `Refresh remote`, `Preview diff`, `Apply visual`, `Apply eligible`, and `Clear`; the tray replaces the need to paste the page-console helper for ordinary manual tests. Auto-refresh is default OFF, uses a hardcoded 6-hour TTL when ON, and only lets `Status` perform TTL-gated maybe-refresh. `Refresh remote` remains manual force refresh. `Preview diff` is debug/preview-only: it compares cached remote LKG against the packaged local catalog, shows compact counts/samples for visual changes, risky changes, unknown remote devices, unknown-device eligibility, and missing local devices, and applies nothing. `Apply visual` remains manual. `Apply eligible` is manual/debug-only and may add eligible unknown remote devices only as in-memory temporary UI cards; it must not enable SAP/material behavior. `Clear` removes cache/LKG/meta/status plus in-memory visual and added-device overlays while preserving Auto-refresh ON/OFF. There is no auto-refresh on panel open, startup/page-load fetch, periodic/scheduled refresh, `chrome.alarms`, or auto-apply on page load.
...
- Next remote-added-device architecture direction is an effective catalog layer: keep the packaged catalog as base/fallback, keep remote-added devices in an in-memory overlay, and first formalize effective helper functions without adding SAP/material behavior. Until a separate material stage is reviewed, `SWAP_MATERIAL_RECYCLE_FILTERS`, `getSwapMaterialRecycleFilter`, `getRecycleDeviceImagePathByMaterialId`, material snapshots, and controlled fill must remain local-only. Later material work should first preview remote-added material eligibility, then add debug-only material enablement, then update docs/checklist after smoke.
