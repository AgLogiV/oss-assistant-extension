...
- Runtime contract reporting is manual/debug status only for now. With the current public JSON, `Status`, `Refresh remote`, or `Preview plan` should show compact compatibility such as `contract legacy ok`; future optional `runtimeContract` data can be reported, but validators are not updated yet and no apply flow is blocked by contract status in this stage.
